...
import LinkTags, { LinkTagEditor } from "./LinkTags";
    ...
    >
      ...
      <div className="link-card-actions">
        <button type="button" className="link-card-action" title="Copy link" aria-label="Copy link" onClick={() => { navigator.clipboard.writeText(link.url); }}>
          <svg width="16" height="16" viewBox="0 0 14 14" fill="none"><rect x="4.5" y="4.5" width="7" height="7" rx="1.5" stroke="currentColor" strokeWidth="1.2"/><path d="M9.5 4.5V3a1.5 1.5 0 00-1.5-1.5H3A1.5 1.5 0 001.5 3v5A1.5 1.5 0 003 9.5h1.5" stroke="currentColor" strokeWidth="1.2"/></svg>
        </button>
        <AnchoredPopover
          open={editingLink}
          ...
          trigger={({ open, toggle }) => (
            <button
              type="button"
              className={`link-card-action${open ? " active" : ""}`}
              title="Edit link"
          ...
          {({ close }) => (
            ...
            >
              ...
              <div className="link-edit-actions">
                <button type="button" className="link-edit-cancel" onClick={close}>Cancel</button>
        ...
        >
          {({ close }) => <LinkTagEditor link={link} allTags={allTags.map(t => t.name)} onClose={close} />}
        </AnchoredPopover>
        <CardMenu link={link} />
      </div>
    </li>
